...
$ration_shop = $_SESSION['userid'];
  ...
  $sql=' SELECT c.*, m.name AS member_name, date_format(c.date , "%d-%M-%Y  - %r") AS ddate FROM `card` c LEFT JOIN member m ON m.card_id = c.card_id WHERE m.type = 1  AND c.login = 1  AND  c.shop_id ='.$ration_shop.'  ORDER BY m.date DESC ';
...
<tr>
    ...
    <td><a href="card_customer.php?id=<?php echo $value['card_id']; ?>" class="btn btn-xs btn-warning"><i class="fa fa-eye" aria-hidden="true"></i></a></td>
