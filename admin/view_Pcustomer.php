...
$ration_shop = $_SESSION['userid'];
    ...
    $sql=' SELECT rs.shop_no AS ration_shopa, c.*, m.name AS member_name, date_format(c.date , "%d-%M-%Y  - %r") AS ddate FROM `card` c LEFT JOIN member m ON m.card_id = c.card_id  LEFT JOIN ration_shop rs ON rs.shop_id = c.shop_id WHERE m.type = 1 AND c.login = 0  ORDER BY m.date DESC ';
...
<tr>
    ...
    <td><a href="edit_customer.php?update=1&id=<?php echo $value['card_id']; ?>" class="btn btn-xs btn-warning"><i class="fa fa-eye" aria-hidden="true"></i></a></td>
